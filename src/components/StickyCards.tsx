...
slideNum,
  ...
  return (
    <div
      className={`md:grid grid-cols-8 flex flex-col md:items-center  px-6 md:h-[85dvh] font-satoshi sticky top-1/3  md:top-20 pt-24 transition-colors duration-500 ${isDark ? "bg-[#191917] text-white border-white" : "bg-[#f3f3f0] text-[#191917] border-[#191917]"}`}
    >
      <div className="col-span-3 md:text-[10.5rem] text-8xl md:h-full font-medium h-fit md:-mt-34">
        {slideNum}
      </div>
      <div className="grid grid-cols-10 col-span-5 h-full border-t pt-5 md:pt-10 pb-20">
        <div className="flex flex-col col-span-4 justify-between items-start h-full">
          <h2 className="md:text-3xl text-sm font-medium uppercase">{topLeftText}</h2>
        ...
        <div className="flex flex-col col-span-5 gap-6 justify-between h-full">
          ...
          <p className=" md:text-3xl text-sm">{bottomRightText}</p>
        </div>
      </div>
    </div>
  );
